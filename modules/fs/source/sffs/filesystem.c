...
SuperBlockInfo *CreateSuperBlock(void)
{
	...
	if (!_superblockInitialized)
	{
		...
		// Try to read generation into Unknown (best-effort)
	...
	{
		u16 val = 0;
		if (i < _fileSystemDataSize >> CLUSTER_SIZE_SHIFT)
			val = SFFSReservedNode;
		else if (i < (_fileSystemDataSize >> CLUSTER_SIZE_SHIFT) * 2)
			val = SFFSFreeNode;
		else
			val = SFFSReservedNode;
...
s32 MarkBlocksReserved(SuperBlockInfo *superblock)
{
	...
	const u32 clustersPerBlock = GetClustersPerBlock();
	const u32 maxBlocks =
	    ((endCluster + blocksShift) >> (SelectedNandSizeInfo.BlockSizeBitShift - 8)) * 3 >> 1;
