...
s32 TryWriteSuperblock()
{
	...
	for(superBlockIndex = 0; !flushed && superBlockIndex < 0x10; superBlockIndex++)
	{
		...
		u32 superblockMetadataOffset = 1 << ((_fileSystemMetadataSizeShift - SelectedNandSizeInfo.BlockSizeBitShift) & 0xFF);
		...
		u32 fatClusterOffset = (SelectedNandSizeInfo.BlockSizeBitShift < (_fileSystemMetadataSizeShift - 4))
			? (superblockMetadataOffset << (_fileSystemMetadataSizeShift - SelectedNandSizeInfo.BlockSizeBitShift) -4 & 0xFF) << fatClusterOffset
