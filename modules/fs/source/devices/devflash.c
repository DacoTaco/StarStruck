...
s32 OpenFlashHandle(void)
{
 // Loop through the available handle slots
	for (u32 i = 0; i < MAX_FLASH_HANDLES; i++)
	{
		...
		// Check if slot is free (NandPosition == 0 and active == 0 means unused)
		if (handle->NandPosition == 0 && handle->IsActive == 0)
		{
			// Allocate the slot
			handle->NandPosition = 0; // Will be set by seek operations
			handle->IsActive = 1; // Mark as active
