...
const NandInformation SupportedNandChips[10] MODULE_DATA = {
 // Hynix HY27US0812(1/2)B
	{ .Info = { .ChipId = 0xAD76,
	  ...
	  .Extension = { .Unknown = 0, .UnknownRegister = 0x01, .Padding = { 0x00, 0x00 } } },
 // Hynix HY27UF081G2A
	{ .Info = { .ChipId = 0xADF1,
	            .Unknown = { 0x00, 0x00 },
	            .Reset = 0xFF,
 ...
 // Hynix HY27UF084G2M or HY27UG084G2M
	{ .Info = { .ChipId = 0xADDC,
	            ...
	            .Padding = { 0x00, 0x00, 0x00 } },
	  .Extension = { .Unknown = 0, .UnknownRegister = 0x00, .Padding = { 0x00, 0x00 } } },
 // Samsung K9F1208U0C
	{ .Info = { .ChipId = 0xEC76,
	            .Unknown = { 0x00, 0x00 },
	            .Reset = 0xFF,
...
void LogCommand(u32 unknown, CommandType commandType, s32 returnValue)
{
  ...
  //oh no, an error to log
		...
		if (InterfaceCommandLog.ErrorIndex == (ERROR_LOG_SIZE - 1))
		{
			...
			InterfaceCommandLog.ErrorOverflowIndex =
			    (InterfaceCommandLog.ErrorOverflowIndex + 1) & (ERROR_LOG_SIZE - 2);
		}
		else
		{
...
s32 InitializeNand()
{
 ...
 //reset/init the interface
	...
	for (; index < 10; index++)
	{
		...
		write32(NAND_UNK1, (read32(NAND_UNK1) & 0xFFFFFFFE) |
		                       SelectedNandChip.Extension.UnknownRegister);
		break;
	}
...
s32 ReadNandPage(u32 pageNumber, void *data, void *ecc, u8 readEcc)
{
	...
	const u32 maxPage = 1 << ((SelectedNandChip.Info.NandSizeBitShift -
	                           SelectedNandChip.Info.PageSizeBitShift) &
	                          0xFF);
	...
	{
		ret = SendNandCommand(SelectedNandChip.Info.ReadPrefix,
		                      SelectedNandChip.Info.InputAddress, 0, 0);
		if (ret != 0)
			goto return_read;
	}
	else
...
return_read:
	LogCommand(pageNumber >> (0xe - SelectedNandChip.Info.PageSizeBitShift),
	           ReadCommand, ret);
	return ret;
}