                             ...
                             u8* data, u32* hmacData)
{
	...
	for (u32 pageIndex = 0; pageIndex < pagesToRead + 2; pageIndex++)
	{
		...
		if (pageIndex != 0 && pageIndex < pagesToRead + 1)
		{
			...
			s32 msgRet = OSReceiveMessage((s32)IrqMessageQueueId, (IpcMessage*)&message, None);
			...
			const bool isLastCluster = (clusterSalt == NULL) || (clusterIndex == count - 1);
			...
			if (hmacBufferPtr != NULL && isInHmacPages && isLastCluster)
			{
				...
				memcpy(hmacBufferPtr, (void*)(eccSlot + eccBufferBase + hmacOffset), hmacChunkSize);
...
#pragma GCC diagnostic pop
		...
		if (pageIndex > 1)
		{
			...
			s32 msgRet = OSReceiveMessage((s32)IoscMessageQueueId, (IpcMessage*)&asyncMessage, None);
		...
		if (pageIndex != 0 && pageIndex < pagesToRead + 1)
		{
			...
			ret = OSIOSCDecryptAsync(KEYRING_CONST_NAND_KEY, clusterIv, pageBuffer, pageSize, pageBuffer,
			                         (s32)IoscMessageQueueId, (IpcMessage*)&asyncMessage);
			if (ret != IPC_SUCCESS)
				goto waitAndReturn;
	...
	while (offset < totalDataSize)
	{
		...
		ret = OSIOSCGenerateBlockMAC(&hmacContext, data + offset, chunkSize, NULL, 0, KEYRING_CONST_NAND_HMAC,
                              ...
                              u8* data, u32* hmacData)
{
	...
	for (u32 pageIndex = 0; pageIndex < totalPages + 1; pageIndex++)
	{
		...
		if (pageIndex > 0)
		{
			...
			ret = WriteNandPage((u32)cluster * pagesPerCluster + prevPageIndex, pageData, eccData, unknownWriteflag, true);
			if (ret != IPC_SUCCESS)
				goto waitAndReturn;
		}
	}
...
s32 WriteClusters(u16 cluster, u32 count, ClusterFlags flags, SaltData* salt, u8* data, u32* hmacData)
{
	...
	errno = WriteClustersInner(cluster, count, clusterIv, clusterSalt, clusterSaltLength, data, hmacData);
...
s32 CheckCluster(u32 cluster)
{
	...
	if (cluster >= GetMaxClusters())
		...
		ret = CheckClusterBlocks(cluster);
