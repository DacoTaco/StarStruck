...
void nand_read_page(u32 pageno, void *data, void *ecc)
{
	...
	if (((s32)data) != -1)
		DCInvalidateRange(data, PAGE_SIZE);
	if (((s32)ecc) != -1)
		DCInvalidateRange(ecc, ECC_BUFFER_SIZE);

	__nand_wait();
	__nand_setup_dma(data, ecc);
	nand_send_command(NAND_READ_POST, 0,
	                  NAND_FLAGS_IRQ | NAND_FLAGS_WAIT | NAND_FLAGS_RD | NAND_FLAGS_ECC,
	                  0x840);
}
...
void nand_write_page(u32 pageno, void *data, void *ecc)
{
	...
	if (((s32)data) != -1)
		DCFlushRange(data, PAGE_SIZE);
	if (((s32)ecc) != -1)
		DCFlushRange(ecc, PAGE_SPARE_SIZE);
	AhbFlushTo(AHB_NAND);
	__nand_set_address(0, pageno);
	__nand_setup_dma(data, ecc);
