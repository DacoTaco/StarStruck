...
#define SEEPROM_ChipSelectLow() clear2(HW_GPIO1OUT, GP_EEP_CS)

#define SEEPROM_ClockEnable() set32(HW_GPIO1OUT, GP_EEP_CLK)
#define SEEPROM_ClockDisable() clear2(HW_GPIO1OUT, GP_EEP_CLK)
...
{
	u32 loword = 0;
	u32 ret = SEEPROM_ReadWord(0x7c, &loword);
	if(ret != IPC_SUCCESS)
		return ret;
...
s32 SEEPROM_BOOT2_GetCounter(BOOT2_Counter* data, s32* counter_write_index)
{
	...
	for(s32 current_counter = 0; current_counter < 2; ++current_counter)
	{
		char* const data_ptr = (char*)&read_data[current_counter];
		u32 value = 0;
		for(s32 i = 0; i < sizeof(BOOT2_Counter); i += sizeof(u16))
		{
	...
	for(s32 current_counter = 0; current_counter < 2; ++current_counter)
	{
		...
		{
			const char* const data_ptr = (const char*)(&read_data[current_counter].UpdateTag);
			const u32 current_value = (u32)(data_ptr[3])
			                          | ((u32)(data_ptr[2]) << 8)
			                          | ((u32)(data_ptr[1]) << 16)
			                          | ((u32)(data_ptr[0]) << 24);
...
s32 SEEPROM_BOOT2_UpdateCounter(const BOOT2_Counter* data, s32 counter_write_index)
{
	...
	u16 value = 0;
	for(s32 i = 0; i < sizeof(*data); i += sizeof(u16))
	{
		memcpy(&value, &data_ptr[i], sizeof(u16));
		ret = SEEPROM_SendWord((0x48 + counter_write_index * sizeof(*data) + i) / sizeof(u16), value);
		if(ret != IPC_SUCCESS)
			break;
	}
...
s32 SEEPROM_NAND_GetCounter(NAND_Counter* data, s32* counter_write_index)
{
	...
	for(s32 current_counter = 0; current_counter < 3; ++current_counter)
	{
		char* const data_ptr = (char*)&read_data[current_counter];
		u32 value = 0;
		for(s32 i = 0; i < sizeof(NAND_Counter); i += sizeof(u16))
		{
	...
	for(s32 current_counter = 0; current_counter < 3; ++current_counter)
	{
		...
		{
			const char* const data_ptr = (const char*)(&read_data[current_counter].NandGen);
			const u32 current_value = (u32)(data_ptr[3])
			                          | ((u32)(data_ptr[2]) << 8)
			                          | ((u32)(data_ptr[1]) << 16)
			                          | ((u32)(data_ptr[0]) << 24);
...
s32 SEEPROM_NAND_UpdateCounter(const NAND_Counter* data, s32 counter_write_index)
{
	...
	{
		memcpy(&value, &data_ptr[i], sizeof(u16));
		ret = SEEPROM_SendWord((0x5c + counter_write_index * sizeof(*data) + i) / sizeof(u16), value);
		if(ret != IPC_SUCCESS)
			break;
	}
